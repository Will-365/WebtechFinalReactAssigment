...
const UserMgtBody = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(`http://localhost:9090/admin/users?pageNo=${currentPage}&pageSize=${pageSize}`);
      const data = await response.json();
      console.log(data); // Log the response to check if the structure is correct
      setUsers(data.users);
  ...
  const handleDelete = async (userId) => {
    try {
      const response = await fetch(`http://localhost:9090/admin/users/delete/${userId}`, {
        method: "POST",
      });
      if (response.ok) {
